...
impl CPU {
    ...
    pub fn run(&mut self) {
        loop {
            ...
            match opscode {
                ...
                // FIXME!
                0x00 => break, // self.brk(&AddressingMode::Implied),
                0xAA => self.tax(),
                0xE8 => self.inx(),
    ...
    fn call(&mut self, op: &OpCode) {
        match op.name.as_str() {
            ...
            "ASL" => {
                self.asl(&op.addressing_mode);
                self.program_counter += op.bytes - 1
            }

            "BCC" => {
                self.bcc(&op.addressing_mode);
                self.program_counter += op.bytes - 1
            }

            "BCS" => {
                self.bcs(&op.addressing_mode);
                self.program_counter += op.bytes - 1
            }

            "BEQ" => {
                self.beq(&op.addressing_mode);
                self.program_counter += op.bytes - 1
            }
            "BIT" => {
            ...
            "STY" => {
                self.sty(&op.addressing_mode);
                self.program_counter += op.bytes - 1
            }

            "TAX" => {
                self.tax(&op.addressing_mode);
                self.program_counter += op.bytes - 1
            }

            "TAY" => {
                self.tay(&op.addressing_mode);
                self.program_counter += op.bytes - 1
            }

            "TSX" => {
                self.tsx(&op.addressing_mode);
                self.program_counter += op.bytes - 1
            }

            "TXA" => {
                self.txa(&op.addressing_mode);
                self.program_counter += op.bytes - 1
            }

            "TXS" => {
                self.txs(&op.addressing_mode);
                self.program_counter += op.bytes - 1
            }

            "TYA" => {
                self.tya(&op.addressing_mode);
                self.program_counter += op.bytes - 1
            }
            _ => {
                todo!();
